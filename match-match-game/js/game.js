class GameInterface {
    ...
    static saveLastLoginDataToLastLoginStorage(userDataObj, name_localStorage) {
        localStorage.setItem(name_localStorage, JSON.stringify(userDataObj));
    }

    static saveUserDataToLocalStorageDataBase(userDataObj, name_localStorage) {
        let dataBase = null;
        ...
        if (GameInterface.isUniqueObjInArrOfObjs(userDataObj, dataBase)) {
            dataBase.push(userDataObj);
            localStorage.setItem(name_localStorage, JSON.stringify(dataBase));
        }
    }

    static isUniqueObjInArrOfObjs(obj, arrOfObjs) {
    ...
    static loadLastUserDataFromStorageToInputs() {
        if (localStorage.getItem("lastLoginUserData")) {
            ...
            const inputs = document.getElementById("user_form").elements;
            inputs.first_name.value = lastUserData.first_name;
            inputs.last_name.value = lastUserData.last_name;
            inputs.email.value = lastUserData.email;
        }
    }
    ...
    static startGame(size) {
        if (!this.cardBack) {
            this.cardBack = "img/cardBacks/cardBack-1.jpg";
        }
        const user_data = GameInterface.getUserDataFromInputs();
        GameInterface.saveLastLoginDataToLastLoginStorage(user_data, "lastLoginUserData");
        GameInterface.saveUserDataToLocalStorageDataBase(user_data, "usersDataBase");
...
class Game {
    constructor(size, cardBack, user_data) {
